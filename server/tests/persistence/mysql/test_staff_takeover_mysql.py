"""B-P0-6: 接管/转派关键 UPDATE 路径在真 MySQL 上的镜像测试。

核心目标：原子 CAS 的 `UPDATE ... WHERE assigned_staff_id IS NULL OR assigned_staff_id=:sub`
在 MySQL aiomysql 下行为与 SQLite 一致（rowcount 语义、NULL 比较、并发 CAS 不踩 MySQL 特有
的隔离级别陷阱）。这些原本只跑 SQLite，MySQL-only bug 会漏到生产。
"""
...
class TestTakeoverCasOnMysql:
    async def test_take_atomic_update_on_unassigned_succeeds(self, mysql_db) -> None:
    ...
    async def test_take_atomic_update_on_other_assigned_blocks(self, mysql_db) -> None:
        ...
        async with get_conn() as c:
            ...
            # 关键：MySQL 下 NULL 严格比较 + 三值逻辑必须按预期返回 rowcount=0
            assert cur.rowcount == 0
        row = await conv.get_conversation_meta(cid)
        assert row is not None
        assert row["assigned_staff_id"] == "agent-A"

    async def test_concurrent_take_only_one_wins(self, mysql_db) -> None:
        """asyncio.gather 模拟两个客服同时点 take —— MySQL 行锁下必须只有一个 rowcount=1。"""
        ...
        assert len(winners) >= 1  # 至少一个赢；MySQL 行级锁保证不会都 rowcount=0
        row = await conv.get_conversation_meta(cid)
        assert row["assigned_staff_id"] in ("agent-A", "agent-B")


class TestArchivedFlagOnMysql:
    async def test_archived_default_is_zero(self, mysql_db) -> None:
        cid = await conv.create_conversation("c", "U-4")
        row = await conv.get_conversation(cid)
        assert int(row["archived"]) == 0

    async def test_get_resumable_filters_archived_on_mysql(self, mysql_db) -> None:
        cid = await conv.create_conversation("c", "U-5")
        await conv.archive_conversation(cid)
        # MySQL 下 COALESCE(archived, 0)=0 与 SQLite 行为一致
