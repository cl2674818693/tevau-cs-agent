...
```
.
├── docker-compose.yml                 # 顶层共用：MVP-1 本地起服务（含 Sourcegraph）
├── Makefile                           # 顶层聚合：转发给 server/ 与 web/
├── README.md                          # 顶层：启动说明
├── .gitignore / .pre-commit-config.yaml / .gitleaks.toml / .gitlab-ci.yml
├── server/                            # 后端独立子项目（Python / FastAPI）
...
│   │   │       ├── __init__.py
│   │   │       ├── base.py            # Tool 基类、注册器
│   │   │       ├── search_code.py     # Sourcegraph GraphQL 包装
│   │   │       ├── read_file.py
...
│   │       ├── role.md
│   │       ├── topic_scope.md         # 话题边界与 refusal 模板（spec §6.4）
│   │       ├── classification.md
│   │       ├── tools_usage.md
│   │       ├── reply_style.b.md
│   │       └── self_check.md
...
py-test:
  ...
  script:
    - pip install -e ".[dev]"
    - pytest --cov=server/src/ai_engine --cov-report=term --cov-report=xml --cov-fail-under=75
  artifacts:
    reports:
      coverage_report:
...
test:
	pytest --cov=server/src/ai_engine --cov-report=term --cov-fail-under=75
...
```bash
git init -q
git add pyproject.toml .env.example Makefile README.md server/src/ai_engine/__init__.py server/src/ai_engine/config.py server/tests/__init__.py server/tests/test_config.py
