...
describe("MessageBubble", () => {
    ...
    it("shows thinking placeholder when assistant content empty", () => {
        render(_jsx(MessageBubble, { m: { role: "assistant", content: "" } }));
        expect(screen.getByText("思考中…")).toBeTruthy();
    });
});
describe("ToolCallChip", () => {
    it("toggles input detail on click", () => {
        render(_jsx(ToolCallChip, { tc: { name: "query_user", input: { user_id: "U1" }, ok: false } }));
...
describe("TicketCard", () => {
    it("shows resolve buttons when resolved and fires callbacks", () => {
        ...
        render(_jsx(TicketCard, { externalId: "AI-1", summary: "\u5361\u7247\u88AB\u9501", status: "resolved", onConfirm: onConfirm, onReject: onReject }));
    ...
    it("renders status label without buttons when pending", () => {
        render(_jsx(TicketCard, { externalId: "AI-2", summary: "\u95EE\u9898", status: "pending" }));
...
describe("HandoffButton", () => {
    it("fires onClick", () => {
        const onClick = vi.fn();
        render(_jsx(HandoffButton, { onClick: onClick, disabled: false }));
